<template lang="html">
  <div class = "login-card">
    ...
    <div class = "log_body">
    <form v-on:submit='addUser($event)'>
...
export default {
  ...
  methods: {
    ...
    if(this.random[0].user == this.newUser){
    alert('Username already exists')
    }
    if(this.random[0].user == 'default') {
      await ToDoAPI.addUser({'user': this.newUser, 'password': this.newPassword})
      if (this.newUser == '') {
        alert('Cannot leave username blank enter again')
        this.newPassword = ''
      }
      if (this.newUser != '') {
